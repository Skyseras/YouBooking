...
export class AuthService {
  ...
  signIn(user:Users):Observable<IdToken|HttpErrorResponse>{
    ...
    return this.http.post<IdToken>("http://localhost:9030/api/login",params).pipe(
      tap(response => {
  ...
  signUp(user:Users):Observable<Users | HttpErrorResponse>{
    this.endPoint = this.role=="CLIENT" ? "signUpUser" : (this.role == "OWNER" ? "signUpManager" : "");
    return this.http.post<Users>("http://localhost:9030/api/"+this.endPoint,user).pipe(
      catchError((error: HttpErrorResponse) => {
        if (error.status === 401) {
          console.error("Error: ", error.error);
          return of(error);
  ...
  refreshToken(){
      ...
      if (token!=null && this.jwtHelper.isTokenExpired(token)){
        if(!this.jwtHelper.isTokenExpired(refreshToken)) {
          ...
          console.log(headers)
          this.http.get<IdToken>("http://localhost:9030/refreshToken", {headers}).subscribe(
            response => {
              localStorage.setItem("access_token", response.access_token.toString());
              localStorage.setItem("refresh_token", response.refresh_token.toString());
              this.isLoggedInSubject.next(true);
